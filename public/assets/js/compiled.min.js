...
c.textAlign="right";c.lineWidth=3;c.strokeStyle="white";c.strokeText(a,32,30);c.fillStyle="black";c.fillText(a,32,30);b.attr("href",d.toDataURL())};e.src=b.attr("href")}else b.attr("href",base_url+"favicon.ico")}function b(){l.scrollTop()>=q?(r.css("margin-top","97px"),m.addClass("fixed-header"),m.css("width",$("#list-nav").parent().width()+"px")):(r.css("margin-top","5px"),m.removeClass("fixed-header"),m.css("width","initial"))}function d(a,b,d){switch(a.status){case 400:alert("ERROR: "+d);break;
...
var e=this;a=$(this).data("mal-id");if("title"!==$(this).data("mal-type")||!$(this).data("mal-id")||confirm("A MAL ID already exists for this series on our backend.\n Are you sure you want to override it?")){var f=prompt("MAL ID:",a);if(/^([0-9]+|none)?$/.test(f)){a=$(this).closest("tr");var g=a.find("td:eq(1)");a=a.attr("data-id");var n=$(g).find(".sprite-myanimelist-net").parent(),h=$(this).find("+ span");""!==f&&"none"!==f&&"0"!==f?c(a,f,function(){n.length?$(n).attr("href","https://myanimelist.net/manga/"+
f):$($("<a/>",{href:"https://myanimelist.net/manga/"+f}).append($("<i/>",{"class":"sprite-site sprite-myanimelist-net",title:f}))).prepend(" ").insertAfter(g.find(".sprite-site"));b($(e),h,f)}):"none"===f||"0"===f?c(a,"0",function(){n.length&&$(n).remove();$($("<a/>",{}).append($("<i/>",{"class":"sprite-site sprite-myanimelist-net-none",title:f}))).prepend(" ").insertAfter(g.find(".sprite-site"));b($(e),h,"none")}):c(a,null,function(){n.remove();h.remove()});$(this).data("mal-id",f)}else null!==f&&
...
0<b.length&&(b=$(b).map(function(){return parseInt($(this).attr("data-id"))}).toArray(),$.post(base_url+"ajax/set_category",{"id[]":b,category:a.attr("value")},function(){location.reload()}).fail(function(a,b,c){d(a,b,c)}))}});if("undefined"!==typeof use_live_countdown_timer&&use_live_countdown_timer&&!/^\/list\//.test(location.pathname))var e=$("#update-timer"),g=e.text().split(":"),h=parseInt(3600*g[0],10)+parseInt(60*g[1],10)+parseInt(g[2],10),k=setInterval(function(){var a=parseInt(h/60/60,10).toString(),
b=parseInt(h/60%60,10).toString(),d=parseInt(h%60,10).toString();1===a.length&&(a="0"+a);1===b.length&&(b="0"+b);1===d.length&&(d="0"+d);e.text(a+":"+b+":"+d);0>--h&&(clearInterval(k),setTimeout(function(){$('link[rel*="icon"]').attr("href",base_url+"favicon.production.updated.ico")},6E4))},1E3);var l=$(window),q=$("#category-nav").offset().top-$("#category-nav").find("> ul").height()-21,m=$("#list-nav"),r=$("table[data-list]");l.scroll(function(){b()});b();$("#update-notice").on("closed.bs.alert",
...
a||"name"in a||"url"in a||"lastChapterReadURL"in a?(d[a.mirror]||(d[a.mirror]=[]),d[a.mirror].push(a),!0):!1}))for(var g in d){if(d.hasOwnProperty(g)){b=d[g];c=a.includes(g)?"#amr_good":"#amr_bad";"#amr_bad"===c&&$("#amr_bad").is(":empty")&&$("<h3/>",{text:"Incompatible Sites"}).appendTo("#amr_bad");$("<h4/>",{text:g+" ("+b.length+")"}).appendTo(c);var h=$("<tbody/>",{"aria-live":"polite","aria-relevant":"all"}),k;for(k in b)if(b.hasOwnProperty(k)){var l=$("<tr/>",{role:"row"}).append($("<td/>",{style:"width: 50%"}).append($("<a/>",
{href:k.url,text:k.name}))).append($("<td/>",{style:"width: 50%"}).append($("<a/>",{href:k.lastChapterReadURL,text:k.lastChapterReadName})));h.append(l)}$("<table/>",{"class":"tablesorter tablesorter-bootstrap table-striped",role:"grid"}).append(h).appendTo(c)}}else alert("JSON has invalid keys?")}else alert('JSON file is missing "mangas" object. Was this exported from AMR?');else alert("ERROR: File isn't valid JSON!")};b.readAsText(c)}else alert("ERROR: Only .json/txt is supported!")})});$(function(){if("index"!==page)return!1;$(".example-image").hover(function(){$(this).attr("src",function(a,c){return c.replace(/\.png$/,".gif")})},function(){$(this).attr("src",function(a,c){return c.replace(/\.gif$/,".png")})})});$(function(){if("login"!==page)return!1;$(".button-checkbox").each(function(){function a(){var a=d.is(":checked");b.data("state",a?"on":"off");b.find(".state-icon").removeClass().addClass("state-icon "+e[b.data("state")].icon);a?b.removeClass("btn-default").addClass("btn-"+f+" active"):b.removeClass("btn-"+f+" active").addClass("btn-default")}var c=$(this),b=c.find("button"),d=c.find("input:checkbox"),f=b.data("color"),e={on:{icon:"glyphicon glyphicon-check"},off:{icon:"glyphicon glyphicon-unchecked"}};
