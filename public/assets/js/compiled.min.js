...
if(a.is("[value]")){var c=$(".tracker-table:visible").find("tr:has(td input[type=checkbox]:checked)");if(0<c.length){var b={id_list:[],category:a.attr("value")};$(c).each(function(){b.id_list.push($(this).attr("data-id"))});a=new FormData;a.append("json",JSON.stringify(b));$.ajax({type:"POST",url:"./ajax/set_category",data:a,success:function(){location.reload()},error:function(a,c,b){},contentType:!1,processData:!1})}}});if("undefined"!==typeof use_live_countdown_timer&&use_live_countdown_timer)var f=
$("#update-timer"),b=f.text().split(":"),d=parseInt(3600*b[0],10)+parseInt(60*b[1],10)+parseInt(b[2],10),h=setInterval(function(){var a=parseInt(d/60/60,10).toString(),c=parseInt(d/60%60,10).toString(),b=parseInt(d%60,10).toString();1==a.length&&(a="0"+a);1==c.length&&(c="0"+c);1==b.length&&(b="0"+b);f.text(a+":"+c+":"+b);0>--d&&clearInterval(h)},1E3)});$(function(){$(".button-checkbox").each(function(){function e(){var c=d.is(":checked");b.data("state",c?"on":"off");b.find(".state-icon").removeClass().addClass("state-icon "+a[b.data("state")].icon);c?b.removeClass("btn-default").addClass("btn-"+h+" active"):b.removeClass("btn-"+h+" active").addClass("btn-default")}var f=$(this),b=f.find("button"),d=f.find("input:checkbox"),h=b.data("color"),a={on:{icon:"glyphicon glyphicon-check"},off:{icon:"glyphicon glyphicon-unchecked"}};b.on("click",function(){d.prop("checked",
