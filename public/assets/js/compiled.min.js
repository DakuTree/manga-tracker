...
$("#tracker-table").find("> tbody > tr > td:nth-of-type(1)").click(function(a){$(a.target).is("input")||(a=$(this).find("> input[type=checkbox]"),$(a).prop("checked",!a.prop("checked")))});$("#delete_selected").click(function(a){a.preventDefault();a=$("#tracker-table").find("tr:has(td input[type=checkbox]:checked)");if(0<a.length){var b=[];$(a).each(function(){b.push($(this).attr("data-id"))});a=new FormData;a.append("json",JSON.stringify(b));$.ajax({type:"POST",url:"./ajax/delete_inline",data:a,
