...
$(function(){$('[data-toggle="tooltip"]').tooltip({html:!0});$(".tablesorter").tablesorter()});$(function(){function a(){var a=$("table[data-list=reading]"),b=a.find("tr .update-read:not([style])").length;a.find("thead > tr > th:eq(1) > div").text("Series"+(0<b?" ("+b+" unread)":""));a.data("unread",b);c(b)}function c(a){a=50<parseInt(a)?"50+":a;var b=$('link[rel="shortcut icon"]');if(0!==parseInt(a)){var d=$("<canvas/>",{id:"faviconCanvas",style:"/*display: none*/"})[0];d.width=32;d.height=32;var c=d.getContext("2d"),e=new Image;e.onload=function(){c.drawImage(e,0,0,32,32);c.font="Bold 17px Helvetica";
c.textAlign="right";c.lineWidth=3;c.strokeStyle="white";c.strokeText(a,32,30);c.fillStyle="black";c.fillText(a,32,30);b.attr("href",d.toDataURL())};e.src=b.attr("href")}else b.attr("href",base_url+"favicon.ico")}function b(){k.scrollTop()>=q?(n.css("margin-top","97px"),m.addClass("fixed-header"),m.css("width",$("#list-nav").parent().width()+"px")):(n.css("margin-top","5px"),m.removeClass("fixed-header"),m.css("width","initial"))}function d(a,b,d){switch(a.status){case 400:alert("ERROR: "+d);break;
case 401:alert("Session has expired, please re-log to continue.");break;case 429:alert("ERROR: Rate limit reached.");break;default:alert("ERROR: Something went wrong!\n"+d)}}if("dashboard"!==page)return!1;$(".tracker-table").find("> tbody > tr > td:nth-of-type(1)").click(function(a){$(a.target).is("input")||(a=$(this).find("> input[type=checkbox]"),$(a).prop("checked",!a.prop("checked")))});$(".update-read").click(function(){var b=$(this).closest("tr"),c=$(b).attr("data-id"),e=$(b).find(".current"),
f=$(b).find(".latest"),p=$(this).parent().find(".update-read, .ignore-latest"),b={id:c,chapter:f.attr("data-chapter")};$.post(base_url+"ajax/update_inline",b,function(){p.hide();$(e).attr("href",$(f).attr("href")).text($(f).text());a()}).fail(function(a,b,c){d(a,b,c)})});$(".ignore-latest").click(function(){var b=$(this).closest("tr"),c=$(b).attr("data-id"),e=$(b).find(".current"),f=$(b).find(".latest"),p=$(this).parent().find(".update-read, .ignore-latest");confirm("Ignore latest chapter?")&&$.post(base_url+
"ajax/ignore_inline",{id:c,chapter:f.attr("data-chapter")},function(){p.hide();$(e).parent().append($("<span/>",{"class":"hidden-chapter",title:"This latest chapter was marked as ignored.",text:$(f).text()}));a()}).fail(function(a,b,c){d(a,b,c)})});$("#delete_selected").click(function(a){a.preventDefault();var b=$(".tracker-table:visible").find("tr:has(td input[type=checkbox]:checked)");a=b.length;0<a?(b=$(b).map(function(){return parseInt($(this).attr("data-id"))}).toArray(),confirm("Are you sure you want to delete the "+
...
a.preventDefault();var e=this;a=$(this).data("mal-id");if("title"!==$(this).data("mal-type")||!$(this).data("mal-id")||confirm("A MAL ID already exists for this series on our backend.\n Are you sure you want to override it?")){var f=prompt("MAL ID:",a);if(/^([0-9]+|none)?$/.test(f)){a=$(this).closest("tr");var h=a.find("td:eq(1)");a=a.attr("data-id");var g=$(h).find(".sprite-myanimelist-net").parent(),k=$(this).find("+ span");""!==f&&"none"!==f&&"0"!==f?c(a,f,function(){g.length?$(g).attr("href",
"https://myanimelist.net/manga/"+f):$($("<a/>",{href:"https://myanimelist.net/manga/"+f}).append($("<i/>",{"class":"sprite-site sprite-myanimelist-net",title:f}))).prepend(" ").insertAfter(h.find(".sprite-site"));b($(e),k,f)}):"none"===f||"0"===f?c(a,"0",function(){g.length&&$(g).remove();$($("<a/>",{}).append($("<i/>",{"class":"sprite-site sprite-myanimelist-net-none",title:f}))).prepend(" ").insertAfter(h.find(".sprite-site"));b($(e),k,"none")}):c(a,null,function(){g.remove();k.remove()});$(this).data("mal-id",
f)}else null!==f&&alert("MAL ID can only contain numbers.")}});$(".more-info").click(function(a){a.preventDefault();$(this).find("+ .tags").toggleClass("has-tags");"More info"===$(this).text()?$(this).text("Hide info"):$(this).text("More info")});$(".edit-tags").click(function(a){a.preventDefault();$(this).parent().find(".tag-edit").toggleClass("hidden")});$(".tag-edit input").on("keypress",function(){13===event.which&&$(this).closest(".tag-edit").find("[type=button]").click()});$(".tag-edit [type=button]").click(function(){var a=
this,b=$(this).closest(".tag-edit").find("input"),c=b.val().trim().replace(/,,/g,","),e=$(this).closest("tr").attr("data-id");if(/^[a-z0-9\-_,:]{0,255}$/.test(c)){var f=Array.from(new Set(c.split(","))).filter(function(a){return""!==a}),g=f.join(",");-1===$.inArray("none",f)?1>=(c.match(/\bmal:(?:[0-9]+|none)\b/g)||[]).length?$.post(base_url+"ajax/tag_update",{id:e,tag_string:g},function(){$(b).val(g);$(a).closest(".tags").find(".tag-list").text(g||"none");$(a).closest(".tag-edit").toggleClass("hidden")}).fail(function(a,
...
0<b.length&&(b=$(b).map(function(){return parseInt($(this).attr("data-id"))}).toArray(),$.post(base_url+"ajax/set_category",{"id[]":b,category:a.attr("value")},function(){location.reload()}).fail(function(a,b,c){d(a,b,c)}))}});if("undefined"!==typeof use_live_countdown_timer&&use_live_countdown_timer&&!/^\/list\//.test(location.pathname))var f=$("#update-timer"),e=f.text().split(":"),g=parseInt(3600*e[0],10)+parseInt(60*e[1],10)+parseInt(e[2],10),l=setInterval(function(){var a=parseInt(g/60/60,10).toString(),
b=parseInt(g/60%60,10).toString(),d=parseInt(g%60,10).toString();1===a.length&&(a="0"+a);1===b.length&&(b="0"+b);1===d.length&&(d="0"+d);f.text(a+":"+b+":"+d);0>--g&&(clearInterval(l),setTimeout(function(){$("link[rel*='icon']").attr("href",base_url+"favicon.production.updated.ico")},6E4))},1E3);var k=$(window),q=$("#category-nav").offset().top-$("#category-nav").find("> ul").height()-21,m=$("#list-nav"),n=$("table[data-list]");k.scroll(function(){b()});b();$("#update-notice").on("closed.bs.alert",
