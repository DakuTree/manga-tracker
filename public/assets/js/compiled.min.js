...
typeof use_live_countdown_timer&&use_live_countdown_timer)var f=$("#update-timer"),b=f.text().split(":"),d=parseInt(3600*b[0],10)+parseInt(60*b[1],10)+parseInt(b[2],10),g=setInterval(function(){var a=parseInt(d/60/60,10).toString(),b=parseInt(d/60%60,10).toString(),c=parseInt(d%60,10).toString();1==a.length&&(a="0"+a);1==b.length&&(b="0"+b);1==c.length&&(c="0"+c);f.text(a+":"+b+":"+c);0>--d&&clearInterval(g)},1E3)});$(function(){if("login"!==page)return!1;$(".button-checkbox").each(function(){function c(){var c=d.is(":checked");b.data("state",c?"on":"off");b.find(".state-icon").removeClass().addClass("state-icon "+a[b.data("state")].icon);c?b.removeClass("btn-default").addClass("btn-"+g+" active"):b.removeClass("btn-"+g+" active").addClass("btn-default")}var f=$(this),b=f.find("button"),d=f.find("input:checkbox"),g=b.data("color"),a={on:{icon:"glyphicon glyphicon-check"},off:{icon:"glyphicon glyphicon-unchecked"}};
b.on("click",function(){d.prop("checked",!d.is(":checked"));d.triggerHandler("change");c()});d.on("change",function(){c()});c();0==b.find(".state-icon").length&&b.prepend('<i class="state-icon '+a[b.data("state")].icon+'"></i>\u00a0')})});$(function(){if("options"!==page)return!1;$("input[type=checkbox][name^=category_custom]").change(function(c){"1"==$(this).data("has-series")&&(alert("Unable to disable category while it still contains series."),$(this).prop("checked",!$(this).prop("checked")))});$("input[type=text][name^=category_custom]").click(function(){$(this).parent().find("[type=checkbox]").attr("checked",!0)})});$(function(){if("signup"!==page)return!1;$("#page[data-page=signup]").find("form").validate({ignore:[],onkeyup:!1,rules:{username:{minlength:4,maxlength:15,pattern:/^[a-zA-Z0-9_-]{4,15}$/,remote:{url:base_url+"ajax/username_check",type:"post",data:{username:function(){return $("input[name='username']").val()}}}}},messages:{username:{minlength:"This username is too short (Minimum 4 characters).",maxlength:"This username is too long (Maximum 15 characters).",pattern:"This username has invalid characters (Only allowed a-z, A-Z, 0-9, _ & - characters).",
remote:"This username already taken."}}});$(".button-checkbox").each(function(){function c(){var c=d.is(":checked");b.data("state",c?"on":"off");b.find(".state-icon").removeClass().addClass("state-icon "+a[b.data("state")].icon);c?b.removeClass("btn-default").addClass("btn-"+g+" active"):b.removeClass("btn-"+g+" active").addClass("btn-default")}var f=$(this),b=f.find("button"),d=f.find("input:checkbox"),g=b.data("color"),a={on:{icon:"glyphicon glyphicon-check"},off:{icon:"glyphicon glyphicon-unchecked"}};
b.on("click",function(){d.prop("checked",!d.is(":checked"));d.triggerHandler("change");c()});d.on("change",function(){c()});c();0==b.find(".state-icon").length&&b.prepend('<i class="state-icon '+a[b.data("state")].icon+'"></i>')})});
