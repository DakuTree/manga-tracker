...
","),f=$(this).closest("tr").attr("data-id");if(/^[a-z0-9\-_,:]{0,255}$/.test(d)){var g=Array.from(new Set(d.split(","))).filter(function(a){return""!==a}),h=g.join(",");-1===$.inArray("none",g)?1>=(d.match(/\bmal:(?:[0-9]+|none)\b/g)||[]).length?$.post(base_url+"ajax/tag_update",{id:f,tag_string:h},function(){$(c).val(h);$(b).closest(".tags").find(".tag-list").text(h||"none");$(b).closest(".tag-edit").toggleClass("hidden")}).fail(function(b,c,d){a(b,c,d)}):alert("You can only use one MAL ID tag per series"):
