...
let sites = {
	...
	'mangadex.com' : extendSite({
		setObjVars : function() {
			...
			this.chapter     = chapter + ':--:' + $('#jump_chapter > option:selected').text().replace(/^Volume ([0-9\.]+).*?Chapter ([0-9\.]+)$/, 'v$1/c$2');

			this.title_url   = `${this.https}://mangadex.com/manga/${this.title}`;
			this.chapter_url = `${this.https}://mangadex.com/chapter/${chapter}`;

			let tempList     = generateChapterList($('#jump_chapter > option'), 'value');
			this.chapterList = Object.keys(tempList).reduce(function(result, key) {
			...
			this.viewerCustomImageList = $('#jump_page > option').map(function(i, e) {
